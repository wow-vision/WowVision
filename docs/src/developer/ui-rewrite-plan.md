...
**Scroll frames (design settled).** Blizzard's virtualized scroll widgets only instantiate visible rows, and Blizzard security means row actions must be genuine hardware clicks on the real row buttons (the auction house above all) — actions never route around the widget through API calls. The adapter therefore pilots the widget: the full logical list comes from the widget's data provider, so every row is a graph node regardless of visibility; row labels come from the row data (a row is spoken the moment focus moves to it, before any scroll, so offscreen frames cannot be read in time); focusing a row scrolls it into view, which materializes its real button; and the click bindings resolve that button lazily at engage time (the activator accepts function-valued targets, and the host runs onFocus before engaging bindings). Identity is the element data itself, so focus follows a row through re-sorts. This is `nodes.scrollBoxList` for the ScrollBox family; FauxScrollFrame and HybridScrollFrame get equivalent offset-math adapters when a migrated window first needs one. The SyncedContainer family and the six proxy scroll classes delete with nothing inheriting their design. Very large lists (auction results) are the one known cost lever: if profiling demands it, emit a windowed band of nodes around focus with explicit position parts.
