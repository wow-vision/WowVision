...
- `nodes.hybridScrollList(builder, { scrollFrame, count, emit, key?, label?, id?, rowHeight? })` — pilots a HybridScrollFrame (a fixed button pool over an API-enumerable list, like the quest log): entries enumerate from the API, focusing an index drives the real scrollbar in row-height units, and the click target is the pooled button whose ID matches the index.
