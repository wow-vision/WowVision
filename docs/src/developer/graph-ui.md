...
Dropdown menus: `core/graph/dropdownMenu.lua` watches the modern Menu manager every frame; any menu that opens anywhere becomes its own stack while it lives (real clicks on the real item buttons; check and radio items read Checked/Unchecked from their mark texture; titles read as text) and closes with it. `Module:registerDropdownMenu(menuKey, emitters)` overrides specific rows: `emitters[index] = function(builder, itemFrame, index)`.
